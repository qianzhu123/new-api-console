...
- Fallback for unrecognized sites: when the JSON contains no usable `session`/`auth_token`, import no longer fails. A `不可导入` placeholder account (provider `unsupported`) is created from the JSON `title` (or the host name), keeping only the name and site URL. Its check-in is marked unsupported and token management is disabled; check-in mode and remarks stay editable in the address detail. Re-import the same site later with credentials, or switch the provider in the edit form, to turn it into a working account. A credential-less import never overwrites an existing working account. When the new-api JWT refresh cookie (`new_api_refresh`, HttpOnly, path-scoped to `/api/user/auth`) is present in the captured JSON, the notes record that it was collected; this version records such sites only and does not implement refresh-token sign-in.
