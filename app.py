...
HISTORY_PATH = ROOT / "quota_history.json"
SIGNIN_PATH = ROOT / "signin_status.json"
...
def request_self_with_retry(session_value: str, user_id: str) -> tuple[requests.Response | None, dict[str, Any], str | None]:
    ...
    headers = build_headers(
        user_id,
        referer="https://www.xiavier.com/console",
        extra_headers={
            "dnt": "1",
            "pragma": "no-cache",
            "connection": "close",
...
def to_public_account(account: dict[str, Any], signin_status: str = "未知") -> dict[str, Any]:
    ...
    return {
        ...
        "signin_status": signin_status or "未知",
    }
...
def build_public_accounts(accounts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for acc in accounts:
        ...
        status = item.get("status") if isinstance(item, dict) else "未知"
        out.append(to_public_account(acc, signin_status=status or "未知"))
    return out
...
def add_account():
    ...
    cfg = save_config(cfg)
    return jsonify({"ok": True, "account": to_public_account(new_account, signin_status="未知"), "accounts": build_public_accounts(cfg["accounts"])})
...
def update_account(name: str):
    ...
    cfg["accounts"] = accounts
    cfg = save_config(cfg)
    return jsonify({"ok": True, "account": to_public_account(updated, signin_status=get_signin_status_today(updated.get("name", ""))), "accounts": build_public_accounts(cfg["accounts"])})
...
def delete_account(name: str):
    ...
    # Keep sign-in store clean for removed accounts.
    with config_lock:
        store = load_signin_store(normalize_and_persist=False)
        amap = store.get("accounts", {})
...
def checkin_one(name: str):
    ...
    if result.get("state") in ("SIGNED_NOW", "ALREADY_SIGNED"):
        ...
        set_signin_status_today(name, "失败")
...
def checkin_all():
    ...
    for acc in accounts:
        ...
        if result.get("state") in ("SIGNED_NOW", "ALREADY_SIGNED"):
            ...
            set_signin_status_today(account_name, "失败")
...
def status_all():
    ...
    accounts = [a for a in cfg.get("accounts", []) if a.get("enabled", True)]
    system_status = fetch_public_status()
    results = [check_status(acc, system_status=system_status) for acc in accounts]
